...
st.set_page_config(page_title="Random Forest Granger Causality (Macro → ETF)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌲 Random Forest Granger Causality</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Non‑linear Granger causality from macro variables to ETFs</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 RF Granger")
...
st.sidebar.markdown(f"**Lag:** {config.LAG} | **Trees:** {config.RF_ESTIMATORS} | **Max depth:** {config.RF_MAX_DEPTH}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rf_granger_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Granger score: {etf['granger_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Granger"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Granger", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌲 Top ETFs by Random Forest Granger Causality (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Random Forest Granger causality** tests whether macro variables non‑linearly improve prediction of ETF returns.
        - We train two RF models: one using only lagged ETF returns, another adding lagged macro variables.
        - The score = improvement in R² when macro variables are included.
        - High score → macro strongly Granger‑causes ETF returns (non‑linear relationship).
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the Granger causality scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Random Forest Granger Causality | Non‑linear macro‑to‑ETF prediction")
